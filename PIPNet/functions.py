...
def forward_pip(net, inputs, preprocess, input_size, net_stride, num_nb):
    ...
    with torch.no_grad():
        ...
        tmp_y = torch.div(max_ids, tmp_width, rounding_mode='trunc').view(-1, 1).float()+outputs_y_select.view(-1, 1)
        ...
        tmp_nb_y = torch.div(max_ids, tmp_width, rounding_mode='trunc').view(-1, 1).float()+outputs_nb_y_select
